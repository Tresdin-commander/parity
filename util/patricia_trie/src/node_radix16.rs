...
pub enum Node<'a> {
	...
	Branch(NibbleSlice<'a>, [&'a [u8]; 16], Option<&'a [u8]>)
}
...
impl<'a> Node<'a> {
	...
	pub fn decoded(node_rlp: &'a [u8]) -> Result<Self, DecoderError> {
		...
		match r.prototype()? {
			...
			Prototype::List(2) => match NibbleSlice::from_encoded(r.at(0)?.data()?) {
				...
				(_, false) => {
                    //Ok(Node::Extension(slice, r.at(1)?.as_raw())),
                    //No extension node so panic
                    panic!("No extension node in radix tree. Somithing went wrong");
                }
			},
			...
			Prototype::List(18) => {
				...
				for i in 1..17 {
					nodes[i-1] = r.at(i)?.as_raw();
				}
                let (key, _) = NibbleSlice::from_encoded(r.at(0)?.data()?);
				Ok(Node::Branch(key, nodes, if r.at(16)?.is_empty() { None } else { Some(r.at(17)?.data()?) }))
			},
	...
	pub fn encoded(&self) -> Bytes {
		match *self {
			...
			// 	stream.out()
			// },
			Node::Branch(ref slice, ref nodes, ref value) => {
